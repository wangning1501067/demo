...
import com.test.demo.user.model.ChinaDate;
...
//from the latest Vocation name
            rst = ChinaDateUtil.latestVocationName;
        } else if (pastTimeFlag == true) {
//更新《当前时间，且最近一次的可见的假期名
            ChinaDateUtil.latestVocationName = rst;
        }
        return rst;
    }
...
//System.out.println(htmlElements.size());
    ...
    public ChinaDate getTodayInfo(String d) {
        List<ChinaDate> dateList = this.getCurrentDateInfo();
        DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
        for (ChinaDate date : dateList) {
            if (dateFormat.format(date.getSolarDate()).equals(d)) {
                return date;
            }
        }
    ...
    public static void main(String[] args) throws FailingHttpStatusCodeException, MalformedURLException, IOException, InterruptedException {
        List<ChinaDate> dateList = new ChinaDateUtil().getCurrentDateInfo();
        ChinaDate today = new ChinaDateUtil().getTodayInfo("20191117");
        DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd");
        ...
        System.out.println("已发生的最近一次假期:" + ChinaDateUtil.latestVocationName);
    }
}
